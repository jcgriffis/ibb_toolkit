function [stacked_results, base_results] = create_stacked_reg_models(model_paths, cfg)
...
base_results.all.mse = cell(size(model_paths));
base_results.all.corr = cell(size(model_paths));
...
base_results.mse = cell(size(model_paths));
base_results.corr = cell(size(model_paths));
for i = 1:length(model_paths)
    ...
    base_results.all.mse{i} = cv_results.all.mse;
    base_results.all.corr{i} = cv_results.all.corr;
...
for i = 1:size(train_set, 2)
    ...
    for j = 1:size(train_set, 3)
        ...
        if contains(cfg.model_spec, 'lasso')
            ...
            if ~isfield(cfg, 'folds') || isempty(cfg.folds)
                cfg.folds = 10;
            end
            if ~isfield(cfg, 'repeats') || isempty(cfg.repeats)
                cfg.repeats = 10;
            end
            [B, fitinfo] = lasso(x_train, y_train, 'Alpha', cfg.Alpha, 'CV', cfg.folds, 'MCReps', cfg.repeats, 'Options', options);
            mdl_idx = fitinfo.IndexMinMSE;
            coef = B(:,mdl_idx);
            coef0 = fitinfo.Intercept(mdl_idx);  
            
            % Get either model predictions or averages of selected features
            if contains(cfg.model_spec, 'pred')
                y_pred = x_test*coef + coef0;
            elseif contains(cfg.model_spec, 'mean')
        ...
        elseif strcmp(cfg.model_spec, 'mean') 
...
% Get collapsed performance (across folds)
